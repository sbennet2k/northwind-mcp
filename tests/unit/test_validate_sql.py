...
@pytest.mark.unit
def test_validate_query_table_not_found(mocker: MockerFixture, mock_schema: DBSchema):
    """Verify error when a table is used that isn't in the schema."""

    mocker.patch("northwind_mcp.server.get_db_schema", return_value=mock_schema)
    mocker.patch("northwind_mcp.server.extract_tables", return_value=["UnknownTable"])
...
def test_validate_query_syntax_error(
    ...
    mocker.patch("northwind_mcp.server.get_db_schema", return_value=mock_schema)
    mocker.patch("northwind_mcp.server.extract_tables", return_value=["FORM"])
...
def test_validate_query_success(
    ...
    mocker.patch("northwind_mcp.server.get_db_schema", return_value=mock_schema)
    mocker.patch(
        "northwind_mcp.server.extract_tables", return_value=["Products", "Categories"]
    )
